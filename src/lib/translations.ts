export type Language = 'en' | 'ta';

export const translations = {
  en: {
    nav: {
      home: "Home",
      services: "Services",
      about: "Dr. Dharmesh",
      gallery: "Gallery",
      qa: "AI Q&A",
      book: "Book Appointment",
      privacy: "Privacy Policy",
      cookies: "Cookie Settings",
      accessibility: "Accessibility",
      patientPrivacy: "Patient Privacy"
    },
    hero: {
      badge: "Chennai's Foremost Integrative Medical Center",
      title1: "Ancient Wisdom",
      title2: "Modern Recovery.",
      subtitle: "Dhanwanthri Maruthuvam integrates traditional Ayurveda with Advanced Sports Therapy and Osteopathic Manipulation for holistic pain management led by Doctor Dharmesh.",
      ctaBook: "WhatsApp Booking",
      ctaServices: "View Services"
    },
    home: {
      overviewTitle: "Dhanwanthri Maruthuvam - Dr. Dharmesh",
      overviewDesc: "A premium Ayurvedic and Sports Therapy clinic located in Chennai, it's the first of its kind in Tamilnadu. Under the guidance of Doctor Dharmesh, we specialize in traditional Ayush medicine paired with advanced techniques like Dry Needling and Osteopathic Manipulation.",
      hours: "Hours",
      hoursVal: "7 AM - 9 PM, Daily",
      capacity: "Capacity",
      capacityVal: "12 Specialized Therapy Rooms",
      servicesTitle: "Specialized Treatments",
      servicesDesc: "Combining evidence-based sports science with holistic rejuvenation for lasting relief, supervised by Dharmesh sports therapist.",
      upgradeTitle: "Exciting Upgrades!",
      upgradeDesc: "We are expanding from a 5-room facility to 12 exclusive therapy rooms, featuring specialized spaces for Panchakarma, HBOT, and Hydrocolon therapy.",
      tourBtn: "Tour Facilities"
    },
    about: {
      badge: "Primary Physician",
      name: "Dr. Dharmesh Kubendiran",
      title: "Doctor Dharmesh - Leading Expert in Integrative Sports Medicine",
      bio1: "Doctor Dharmesh (Dr. Dharmesh Kubendiran) is a visionary in the field of Ayush medicine, bridging the gap between ancient Ayurvedic healing and modern sports science. With a BAMS degree and a PhD in Sports Method and Training, Dharmesh doctor brings a unique scientific perspective to traditional therapies.",
      bio2: "As a dedicated Dharmesh sports therapist and Biomechanist, Dr. Dharmesh focuses on analyzing gait patterns, muscle mechanics, and neurological communication to resolve chronic pain at its source.",
      philosophy: "Professional Philosophy",
      quote: "\"Our approach isn't just about suppressing symptoms; it's about optimizing the human machine. By integrating Kinesiology with Siddha and Ayurveda, we reset the body's natural healing systems and restore peak functional performance.\"",
      expertiseTitle: "Key Expertise:",
      exp1: "Spinal Disorders & Arthritis",
      exp2: "Neuromusculoskeletal Pain",
      exp3: "Gait Pattern Correction",
      exp4: "Advanced Sports Rehabilitation",
      exp5: "Myofascial Trigger Therapy",
      exp6: "Metabolic Recovery"
    },
    services: {
      badge: "Our Expertise",
      title: "Advanced Therapeutic Services",
      desc: "Combining traditional Ayush medicine with modern sports science for precise, personalized care directed by Doctor Dharmesh.",
      kinesiology: {
        title: "Advanced Kinesiology",
        desc: "The scientific study of human body movement. It integrates human anatomy, physiology, and biomechanics to understand function and health.",
        p1: "Correcting Muscle Imbalances",
        p2: "Neuromuscular Re-education",
        p3: "Biomechanical Optimization",
        p4: "Lymphatic and Vascular Support",
        p5: "Integration of Holistic Factors"
      },
      needling: {
        title: "Dry Needling",
        desc: "A modern clinical treatment based on neurobiology. Using thin filiform needles to target myofascial trigger points.",
        p1: "Rapid Pain Relief",
        p2: "Improved Range of Motion",
        p3: "Accelerated Healing",
        p4: "Neurological Reset",
        p5: "Recovery for Athletes"
      },
      manipulation: {
        title: "Therapeutic Manipulation",
        desc: "Controlled HVLA thrust techniques applied to joints to restore range of motion and neurological health.",
        p1: "Spinal Manipulation",
        p2: "Peripheral Joint Manipulation",
        p3: "Osteopathic Manual Manipulation",
        p4: "Muscle Energy Technique",
        p5: "Instrument-Assisted Manipulation"
      },
      hbot: {
        title: "Specialized Care",
        desc: "Upgraded facilities for advanced holistic and recovery treatments including 2ATA HBOT.",
        p1: "HBOT 2ATA Chamber",
        p2: "Hydrocolon Therapy",
        p3: "Traditional Panchakarma",
        p4: "Sports Rehabilitation",
        p5: "Pain Management"
      }
    },
    gallery: {
      badge: "Visual Tour",
      title: "Clinic & Procedures",
      desc: "Experience the sophisticated environment and precision-driven treatments at Dhanwanthri Maruthuvam.",
      videoTitle: "Video Demonstrations",
      v1Title: "Advanced Kinesiology Overview",
      v1Sub: "Clinical Tutorial",
      v2Title: "Clinic Facility Walkthrough",
      v2Sub: "Facility Tour",
      facilityTitle: "Facility Gallery",
      viewFull: "View Fullscreen",
      cat1: "Interior",
      cat2: "Treatment",
      cat3: "Consultation",
      cat4: "Clinical",
      item1: "Clinic Reception",
      item2: "Traditional Therapy Room",
      item3: "Clinical Procedure",
      item4: "Modern clinic space-1",
      item5: "Patient waiting area",
      item6: "Therapy Consultation"
    },
    qa: {
      badge: "AI Assistant",
      title: "Healing Insights Assistant",
      desc: "Ask our AI about our treatments, specific conditions, or clinical methodology.",
      cardTitle: "Virtual Consultant",
      cardDesc: "Get immediate answers based on our clinical expertise.",
      placeholder: "Type your question here...",
      suggested: "Suggested Questions:",
      insight: "Our Insights:",
      error: "I'm sorry, I encountered an error. Please try again later.",
      q1: "How can kinesiology help with my back pain?",
      q2: "What are the benefits of HBOT?",
      q3: "Difference between dry needling and acupuncture?",
      q4: "How does joint manipulation work?",
      q5: "Who is Dr. Dharmesh Kubendiran?"
    },
    contact: {
      badge: "Reach Out",
      title: "Get in Touch",
      desc: "Whether you have questions about specific treatments or wish to book a consultation with Doctor Dharmesh, we are here to help.",
      loc: "Location",
      locVal: "No.2, 54th street lane, 11th avenue, Ashok Nagar, Chennai 600083",
      viewMap: "View on Map",
      phone: "Phone",
      phoneVal: "8608174673",
      phoneSub: "Click to call for appointments",
      email: "Email Address",
      emailVal: "admin@dhanwanthrimaruthuvam.com",
      emailSub: "For records and inquiries",
      hours: "Clinic Hours",
      hoursVal: "7:00 AM — 9:00 PM",
      hoursSub: "Monday through Sunday",
      social: "Social Links",
      formTitle: "Book Appointment",
      formDesc: "Fill out the form below or message us on WhatsApp.",
      name: "Full Name",
      phoneLabel: "Phone Number",
      emailLabel: "Your Email",
      service: "Preferred Service",
      message: "Your Inquiry / Symptoms",
      submit: "Submit Inquiry",
      successTitle: "Inquiry Sent!",
      successDesc: "Thank you for reaching out. Our clinical coordinators will contact you within 24 hours.",
      successBtn: "Send another inquiry",
      whatsappMsg: "Hi, I'd like to book an appointment with Doctor Dharmesh at Dhanwanthri Maruthuvam."
    },
    footer: {
      ctaTitle: "Ready to start your recovery?",
      ctaSub: "Expert consultation with Doctor Dharmesh available 7 days a week.",
      ctaBook: "Book Appointment",
      ctaAI: "Ask Our AI",
      mission: "Bridging the gap between ancient Ayurvedic wisdom and modern sports medicine. Our mission is to restore peak human performance through scientific movement analysis and holistic therapies led by Dr. Dharmesh.",
      treatments: "Treatments",
      info: "Information",
      connect: "Connect",
      accredited: "Accredited Facility"
    }
  },
  ta: {
    nav: {
      home: "முகப்பு",
      services: "சேவைகள்",
      about: "டாக்டர் தர்மேஷ்",
      gallery: "கேலரி",
      qa: "AI கேள்வி-பதில்",
      book: "முன்பதிவு",
      privacy: "தனியுரிமைக் கொள்கை",
      cookies: "குக்கீ அமைப்புகள்",
      accessibility: "அணுகல்தன்மை",
      patientPrivacy: "நோயாளி தனியுரிமை"
    },
    hero: {
      badge: "சென்னையின் முதன்மை ஒருங்கிணைந்த மருத்துவ மையம்",
      title1: "பண்டைய ஞானம்",
      title2: "நவீன மீட்பு.",
      subtitle: "டாக்டர் தர்மேஷ் தலைமையில் தன்வந்திரி மருத்துவம் பாரம்பரிய ஆயுர்வேதத்தை மேம்பட்ட விளையாட்டு சிகிச்சை மற்றும் ஆஸ்டியோபதி கையாளுதலுடன் ஒருங்கிணைத்து முழுமையான வலி மேலாண்மையை வழங்குகிறது.",
      ctaBook: "வாட்ஸ்அப் முன்பதிவு",
      ctaServices: "சேவைகளைப் பார்க்க"
    },
    home: {
      overviewTitle: "தன்வந்திரி மருத்துவம் - டாக்டர் தர்மேஷ்",
      overviewDesc: "சென்னையில் அமைந்துள்ள ஒரு பிரீமியம் ஆயுர்வேத மற்றும் விளையாட்டு சிகிச்சை கிளினிக், இது தமிழ்நாட்டில் முதல் முறையாகும். டாக்டர் தர்மேஷ் வழிகாட்டுதலின் கீழ், டிரை நீட்லிங் மற்றும் ஆஸ்டியோபதி கையாளுதல் போன்ற மேம்பட்ட நுட்பங்களுடன் பாரம்பரிய ஆயுஷ் மருத்துவத்தில் நாங்கள் நிபுணத்துவம் பெற்றுள்ளோம்.",
      hours: "நேரம்",
      hoursVal: "காலை 7 - இரவு 9, தினமும்",
      capacity: "வசதி",
      capacityVal: "12 சிறப்பு சிகிச்சை அறைகள்",
      servicesTitle: "சிறப்பு சிகிச்சைகள்",
      servicesDesc: "நீடித்த நிவாரணத்திற்காக ஆதார அடிப்படையிலான விளையாட்டு அறிவியலை முழுமையான புத்துணர்ச்சியுடன் இணைத்தல்.",
      upgradeTitle: "சிறப்பான மேம்பாடுகள்!",
      upgradeDesc: "நாங்கள் 5 அறைகள் கொண்ட வசதியிலிருந்து 12 பிரத்யேக சிகிச்சை அறைகளாக விரிவுபடுத்துகிறோம், இதில் பஞ்சகர்மா, HBOT மற்றும் ஹைட்ரோகோலன் சிகிச்சைக்கான சிறப்பு இடங்கள் உள்ளன.",
      tourBtn: "வசதிகளைப் பார்க்க"
    },
    about: {
      badge: "முதன்மை மருத்துவர்",
      name: "டாக்டர் தர்மேஷ் குபேந்திரன்",
      title: "டாக்டர் தர்மேஷ் - ஒருங்கிணைந்த விளையாட்டு மருத்துவத்தில் முன்னணி நிபுணர்",
      bio1: "டாக்டர் தர்மேஷ் குபேந்திரன் ஆயுஷ் மருத்துவத் துறையில் ஒரு தொலைநோக்கு பார்வையாளர், பண்டைய ஆயுர்வேத குணப்படுத்துதலுக்கும் நவீன விளையாட்டு அறிவியலுக்கும் இடையிலான இடைவெளியைக் குறைக்கிறார். BAMS பட்டம் மற்றும் விளையாட்டு முறை மற்றும் பயிற்சியில் PhD பெற்றுள்ள தர்மேஷ் டாக்டர், பாரம்பரிய சிகிச்சைகளுக்கு ஒரு தனித்துவமான அறிவியல் கண்ணோட்டத்தைக் கொண்டுவருகிறார்.",
      bio2: "ஒரு அர்ப்பணிப்புள்ள தர்மேஷ் விளையாட்டு சிகிச்சையாளர் மற்றும் பயோமெக்கானிஸ்ட் ஆக, டாக்டர் தர்மேஷ் நாள்பட்ட வலியை அதன் மூலத்திலேயே தீர்க்க நடை முறைகள், தசை இயக்கவியல் மற்றும் நரம்பியல் தொடர்புகளை பகுப்பாய்வு செய்வதில் கவனம் செலுத்துகிறார்.",
      philosophy: "தொழில்முறை தத்துவம்",
      quote: "\"எங்கள் அணுகுமுறை அறிகுறிகளை அடக்குவது மட்டுமல்ல; இது மனித இயந்திரத்தை மேம்படுத்துவது பற்றியது. கினீசியாலஜியை சித்தா மற்றும் ஆயுர்வேதத்துடன் ஒருங்கிணைப்பதன் மூலம், உடலின் இயற்கையான குணப்படுத்தும் அமைப்புகளை மீட்டமைத்து, உச்ச செயல்பாட்டு செயல்திறனை மீட்டெடுக்கிறோம்.\"",
      expertiseTitle: "முக்கிய நிபுணத்துவம்:",
      exp1: "முதுகுத்தண்டு கோளாறுகள் & மூட்டுவலி",
      exp2: "நரம்புத்தசை வலி",
      exp3: "நடை முறை திருத்தம்",
      exp4: "மேம்பட்ட விளையாட்டு மறுவாழ்வு",
      exp5: "மயோஃபாஸியல் தூண்டுதல் சிகிச்சை",
      exp6: "மெட்டபாலிக் மீட்பு"
    },
    services: {
      badge: "எங்கள் நிபுணத்துவம்",
      title: "மேம்பட்ட சிகிச்சை சேவைகள்",
      desc: "துல்லியமான, தனிப்பயனாக்கப்பட்ட கவனிப்புக்காக டாக்டர் தர்மேஷ் வழிகாட்டுதலின் கீழ் பாரம்பரிய ஆயுஷ் மருத்துவத்தை நவீன விளையாட்டு அறிவியலுடன் இணைத்தல்.",
      kinesiology: {
        title: "மேம்பட்ட கினீசியாலஜி",
        desc: "மனித உடல் இயக்கத்தின் அறிவியல் ஆய்வு. இது செயல்பாட்டையும் ஆரோக்கியத்தையும் புரிந்துகொள்ள உடற்கூறியல் மற்றும் உடலியல் ஆகியவற்றை ஒருங்கிணைக்கிறது.",
        p1: "தசை ஏற்றத்தாழ்வுகளை திருத்துதல்",
        p2: "நரம்புத்தசை மறு கல்வி",
        p3: "பயோமெக்கானிக்கல் உகப்பாக்கம்",
        p4: "நிணநீர் மற்றும் வாஸ்குலர் ஆதரவு",
        p5: "முழுமையான காரணிகளின் ஒருங்கிணைப்பு"
      },
      needling: {
        title: "டிரை நீட்லிங்",
        desc: "நரம்பியல் அறிவியலை அடிப்படையாகக் கொண்ட நவீன மருத்துவ சிகிச்சை. தசை நார்களை இலக்காகக் கொண்டு மெல்லிய ஊசிகளைப் பயன்படுத்துதல்.",
        p1: "விரைவான வலி நிவாரணம்",
        p2: "மேம்பட்ட இயக்க வரம்பு",
        p3: "முடுக்கிவிடப்பட்ட குணப்படுத்துதல்",
        p4: "நரம்பியல் மீட்டமைப்பு",
        p5: "விளையாட்டு வீரர்களுக்கான மீட்பு"
      },
      manipulation: {
        title: "சிகிச்சை கையாளுதல்",
        desc: "இயக்க வரம்பு மற்றும் நரம்பியல் ஆரோக்கியத்தை மீட்டெடுக்க மூட்டுகளில் பயன்படுத்தப்படும் கட்டுப்படுத்தப்பட்ட HVLA நுட்பங்கள்.",
        p1: "முதுகுத்தண்டு கையாளுதல்",
        p2: "சுற்றயல் மூட்டு கையாளுதல்",
        p3: "ஆஸ்டியோபதி கையேடு கையாளுதல்",
        p4: "தசை ஆற்றல் நுட்பம்",
        p5: "கருவி-உதவி கையாளுதல்"
      },
      hbot: {
        title: "சிறப்பு கவனிப்பு",
        desc: "2ATA HBOT உள்ளிட்ட மேம்பட்ட முழுமையான மற்றும் மீட்பு சிகிச்சைகளுக்கான மேம்படுத்தப்பட்ட வசதிகள்.",
        p1: "HBOT 2ATA அறை",
        p2: "ஹைட்ரோகோலன் சிகிச்சை",
        p3: "பாரம்பரிய பஞ்சகர்மா",
        p4: "விளையாட்டு மறுவாழ்வு",
        p5: "வலி மேலாண்மை"
      }
    },
    gallery: {
      badge: "காட்சிப் பயணம்",
      title: "கிளினிக் மற்றும் நடைமுறைகள்",
      desc: "தன்வந்திரி மருத்துவத்தில் அதிநவீன சூழல் மற்றும் துல்லியமான சிகிச்சைகளை அனுபவியுங்கள்.",
      videoTitle: "வீடியோ விளக்கங்கள்",
      v1Title: "மேம்பட்ட கினீசியாலஜி கண்ணோட்டம்",
      v1Sub: "மருத்துவ பயிற்சி",
      v2Title: "கிளினிக் வசதி நடைபயணம்",
      v2Sub: "வசதி பயணம்",
      facilityTitle: "வசதி கேலரி",
      viewFull: "முழுத்திரையில் பார்க்க",
      cat1: "உட்புறம்",
      cat2: "சிகிச்சை",
      cat3: "ஆலோசனை",
      cat4: "மருத்துவம்",
      item1: "கிளினிக் வரவேற்பு",
      item2: "பாரம்பரிய சிகிச்சை அறை",
      item3: "மருத்துவ நடைமுறை",
      item4: "நவீன கிளினிக் இடம்-1",
      item5: "நோயாளி காத்திருப்பு பகுதி",
      item6: "சிகிச்சை ஆலோசனை"
    },
    qa: {
      badge: "AI உதவியாளர்",
      title: "ஹீலிங் நுண்ணறிவு உதவியாளர்",
      desc: "எங்கள் சிகிச்சைகள், குறிப்பிட்ட நிலைமைகள் அல்லது மருத்துவ முறைகள் பற்றி எங்கள் AI-யிடம் கேளுங்கள்.",
      cardTitle: "மெய்நிகர் ஆலோசகர்",
      cardDesc: "எங்கள் மருத்துவ நிபுணத்துவத்தின் அடிப்படையில் உடனடி பதில்களைப் பெறுங்கள்.",
      placeholder: "உங்கள் கேள்வியை இங்கே தட்டச்சு செய்யவும்...",
      suggested: "பரிந்துரைக்கப்பட்ட கேள்விகள்:",
      insight: "எங்கள் நுண்ணறிவு:",
      error: "மன்னிக்கவும், ஒரு பிழை ஏற்பட்டது. பின்னர் மீண்டும் முயற்சிக்கவும்.",
      q1: "கினீசியாலஜி எனது முதுகுவலிக்கு எப்படி உதவும்?",
      q2: "HBOT-ன் நன்மைகள் என்ன?",
      q3: "டிரை நீட்லிங் மற்றும் அக்குபஞ்சர் இடையே உள்ள வேறுபாடு?",
      q4: "மூட்டு கையாளுதல் எவ்வாறு செயல்படுகிறது?",
      q5: "டாக்டர் தர்மேஷ் குபேந்திரன் யார்?"
    },
    contact: {
      badge: "தொடர்பு கொள்ள",
      title: "தொடர்பில் இருங்கள்",
      desc: "குறிப்பிட்ட சிகிச்சைகள் பற்றிய கேள்விகள் இருந்தாலும் அல்லது டாக்டர் தர்மேஷிடம் ஆலோசனையைப் பதிவு செய்ய விரும்பினாலும், நாங்கள் உதவ இருக்கிறோம்.",
      loc: "இடம்",
      locVal: "எண்.2, 54வது தெரு சந்து, 11வது அவென்யூ, அசோக் நகர், சென்னை 600083",
      viewMap: "வரைபடத்தில் பார்க்க",
      phone: "தொலைபேசி",
      phoneVal: "8608174673",
      phoneSub: "முன்பதிவு செய்ய கிளிக் செய்யவும்",
      email: "மின்னஞ்சல்",
      emailVal: "admin@dhanwanthrimaruthuvam.com",
      emailSub: "பதிவுகள் மற்றும் விசாரணைகளுக்கு",
      hours: "கிளினிக் நேரம்",
      hoursVal: "காலை 7:00 — இரவு 9:00",
      hoursSub: "திங்கள் முதல் ஞாயிறு வரை",
      social: "சமூக இணைப்புகள்",
      formTitle: "முன்பதிவு",
      formDesc: "படிவத்தை நிரப்பவும் அல்லது வாட்ஸ்அப் மூலம் எங்களுக்கு செய்தி அனுப்பவும்.",
      name: "முழு பெயர்",
      phoneLabel: "தொலைபேசி எண்",
      emailLabel: "உங்கள் மின்னஞ்சல்",
      service: "விருப்பமான சேவை",
      message: "உங்கள் விசாரணை / அறிகுறிகள்",
      submit: "விசாரணையைச் சமர்ப்பிக்கவும்",
      successTitle: "விசாரணை அனுப்பப்பட்டது!",
      successDesc: "தொடர்பு கொண்டதற்கு நன்றி. எங்கள் மருத்துவ ஒருங்கிணைப்பாளர்கள் 24 மணி நேரத்திற்குள் உங்களைத் தொடர்புகொள்வார்கள்.",
      successBtn: "மற்றொரு விசாரணையை அனுப்பவும்",
      whatsappMsg: "வணக்கம், தர்மேஷ் டாக்டரிடம் தன்வந்திரி மருத்துவத்தில் ஒரு அப்பாயிண்ட்மெண்ட் பதிவு செய்ய விரும்புகிறேன்."
    },
    footer: {
      ctaTitle: "உங்கள் மீட்சியைத் தொடங்கத் தயாரா?",
      ctaSub: "டாக்டர் தர்மேஷ் ஆலோசனை வாரம் 7 நாட்களும் கிடைக்கும்.",
      ctaBook: "அப்பாயிண்ட்மெண்ட் பதிவு",
      ctaAI: "எங்கள் AI-யிடம் கேளுங்கள்",
      mission: "பண்டைய ஆயுர்வேத ஞானத்திற்கும் நவீன விளையாட்டு மருத்துவத்திற்கும் இடையிலான இடைவெளியைக் டாக்டர் தர்மேஷ் தலைமையில் குறைத்தல். அறிவியல் பூர்வமான இயக்க பகுப்பாய்வு மற்றும் முழுமையான சிகிச்சைகள் மூலம் மனித செயல்திறனை மீட்டெடுப்பதே எங்கள் நோக்கம்.",
      treatments: "சிகிச்சைகள்",
      info: "தகவல்",
      connect: "தொடர்பு கொள்ள",
      accredited: "அங்கீகரிக்கப்பட்ட வசதி"
    }
  }
};
